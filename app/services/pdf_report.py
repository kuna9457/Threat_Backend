...
def generate_pdf_report(results: list[dict]) -> bytes:
    ...
    story.append(Paragraph("🛡️ URL Threat Intelligence Report", title_style))
    ...
    for idx, res in enumerate(results, 1):
        ...
        # URL Header
        story.append(Paragraph(
            f"#{idx} — {_esc(res.get('url', ''))}", heading_style
        ))
        story.append(Paragraph(
            f"Scanned: {res.get('timestamp', '—')}", small_style
        ))
        story.append(HRFlowable(
        ...
        vt_rows = [
            ...
            ["Reputation", str(vt.get("reputation", "—"))],
        ]
        ...
        if urlscan and "error" not in urlscan:
            ...
            us_rows = [
                ...
                ["Country", str(urlscan.get("country", "—"))],
                ["Server", str(urlscan.get("server", "—"))],
            ]
        ...
        if abuseipdb and "error" not in abuseipdb:
            ...
            ab_rows = [
                ["Metric", "Value"],
                ["Resolved IP", str(abuseipdb.get("ip", "—"))],
        ...
        if ssl and "error" not in ssl:
            ...
            ssl_rows = [
                ...
                ["IP Address", ssl.get("ip_address", "—")],
                ["Server Name", ssl.get("server_name", "—")],
            ]
            if cert:
                ssl_rows += [
                    ...
                    ["Signature Algorithm", str(cert.get("sig_alg", "—"))],
                ]
                # Validity dates
            ...
            story.append(Paragraph(f"⚠ {ssl['error']}", body_style))
        ...
        if mx and "error" not in mx:
            ...
            if ms:
                ...
                for check_name, check_info in checks.items():
                    status_icon = "✅" if check_info.get("status") == "PASS" else (
                        "⚠️" if check_info.get("status") == "WARN" else "❌"
                    )
                    story.append(Paragraph(
            ...
            story.append(Paragraph("MX / DNS Analysis", subheading_style))
            story.append(Paragraph(f"⚠ {mx['error']}", body_style))
